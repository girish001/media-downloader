...
if pip3 install --upgrade --break-system-packages yt-dlp > /dev/null 2>&1; then
  ...
  echo "[worker-entrypoint] yt-dlp updated via pip: $($YTDLP_PATH --version 2>/dev/null)"
else
  echo "[worker-entrypoint] WARNING: pip3 update failed, using installed version"
  export YTDLP_PATH=$(which yt-dlp 2>/dev/null || echo '/usr/local/bin/yt-dlp')
fi
echo "[worker-entrypoint] Deno: $(deno --version 2>/dev/null | head -1 || echo 'not available')"
echo "[worker-entrypoint] Node.js: $(node --version 2>/dev/null || echo 'not available')"
