...
function mapFormatToYtdlp(format: string, platform?: string): string {
  ...
  const map: Record<string, string> = {
    '144p':  'bv*[height<=144]+ba/b[height<=144]/b',
    ...
    'mp3':   'ba/b',
    'hd':    'bv*[height<=1080]+ba/b[height<=1080]/b',
    'sd':    'bv*[height<=480]+ba/b[height<=480]/b',
    'best':  'bv*+ba/b',
  };
...
async function processJob(job: Job): Promise<{
  ...
  if (platform === 'youtube') {
    ...
    //   mweb         → FINAL fallback
    // player_skip=webpage,configs,js skips JS player download to avoid bot fingerprint.
    ytdlpArgs.push(
      '--extractor-args', 'youtube:player_client=tv_embedded,web_embedded,ios,web_creator,mweb;player_skip=webpage,configs,js',
      '--geo-bypass',
    );
  }
