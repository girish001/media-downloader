...
function mapFormatToYtdlp(format: string, platform?: string): string {
  ...
  // Format selector strategy for YouTube downloads:
  // 1. bv*[height<=N]+ba  — best DASH video + audio (highest quality, requires merge)
  // 2. b[height<=N][ext=mp4] — progressive MP4 (pre-muxed, no nsig issues, good fallback)
  // 3. b[height<=N]       — any pre-muxed stream at height
  // 4. b                  — absolute fallback: best available anything
  //
  // The progressive MP4 fallback (#2) is critical for avoiding HTTP 403 errors:
  // DASH streams have an "n" parameter that yt-dlp must decode via the YouTube JS player.
  // If the nsig decoding fails (stale JS player cache, outdated yt-dlp), YouTube CDN
  // returns 403 Forbidden. Progressive streams do NOT have the n-parameter → no 403.
  const map: Record<string, string> = {
    '144p':  'bv*[height<=144]+ba/b[height<=144][ext=mp4]/b[height<=144]/b',
    '240p':  'bv*[height<=240]+ba/b[height<=240][ext=mp4]/b[height<=240]/b',
    '360p':  'bv*[height<=360]+ba/b[height<=360][ext=mp4]/b[height<=360]/b',
    '480p':  'bv*[height<=480]+ba/b[height<=480][ext=mp4]/b[height<=480]/b',
    '720p':  'bv*[height<=720]+ba/b[height<=720][ext=mp4]/b[height<=720]/b',
    '1080p': 'bv*[height<=1080]+ba/b[height<=1080][ext=mp4]/b[height<=1080]/b',
    '1440p': 'bv*[height<=1440]+ba/b[height<=1440][ext=mp4]/b[height<=1440]/b',
    '4k':    'bv*[height<=2160]+ba/b[height<=2160][ext=mp4]/b[height<=2160]/b',
    'mp3':   'ba/b',
    'hd':    'bv*[height<=1080]+ba/b[height<=1080][ext=mp4]/b[height<=1080]/b',
    'sd':    'bv*[height<=480]+ba/b[height<=480][ext=mp4]/b[height<=480]/b',
    'best':  'bv*+ba/b[ext=mp4]/b',
  };
  return map[format.toLowerCase()] ?? 'bv*+ba/b';
}
...
async function processJob(job: Job): Promise<{
  ...
  if (platform === 'youtube') {
    // Cookies FIRST — must come before extractor-args so the 'web' client
    // in the chain can authenticate immediately on its first attempt.
    // Decoded from YT_COOKIES_BASE64 env var at container startup.
    if (process.env.YT_COOKIES_FILE) {
      ytdlpArgs.push('--cookies', process.env.YT_COOKIES_FILE);
    }
    // Realistic Android UA reduces YouTube bot-score on Railway GCP IPs
    ytdlpArgs.push(
      '--add-header',
      'User-Agent:Mozilla/5.0 (Linux; Android 13; Pixel 7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Mobile Safari/537.36',
    );

    // Player client chain — ordered for Railway/datacenter IP reliability (2025).
    //
    // ROOT CAUSE OF "Requested format is not available":
    // YouTube detects Railway/GCP datacenter IPs and blocks specific clients.
    // The chain below leads with clients YouTube permits for server-side access.
    //
    // FIXED CLIENT ORDER for 2026 Railway/datacenter IPs:
    // Lead with EMBED clients — YouTube permits server-side use, no po_token.
    //   tv_embedded  → PRIMARY (YouTube TV, server-side permitted)
    //   web_embedded → SECONDARY (iframe embed, server-side permitted)
    //   ios          → TERTIARY (AppAttest auth, not browser-po_token restricted)
    //   web_creator  → QUATERNARY (may require po_token in some regions)
    //   mweb         → FINAL fallback
    ytdlpArgs.push(
      '--extractor-args', 'youtube:player_client=default,tv_embedded,web_embedded,ios,mweb',
      '--geo-bypass',
      // Prevent stale JS player cache from causing nsig decode failures (HTTP 403)
      '--no-cache-dir',
    );
  }
