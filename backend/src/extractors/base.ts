...
function translateYtdlpError(err: any): string {
  ...
  if (/copyright/i.test(raw))
    return 'This video cannot be downloaded due to a copyright/music label restriction. Major Indian music labels (Tips Music, T-Series, Sony Music India) block server-side downloads.';
  ...
  if (/Sign in to confirm your age|age.?restrict/i.test(raw))
    return 'This video is age-restricted. If you have set YT_COOKIES_BASE64, ensure the cookies include HttpOnly cookies (use "Get cookies.txt LOCALLY" Chrome extension, NOT EditThisCookie).';

  if (/Sign in to confirm/i.test(raw))
    return 'YouTube bot-detection triggered. This can happen with major music label videos (Tips Music, T-Series, Sony Music) which have extra download restrictions, OR your cookies may have expired. Try refreshing YT_COOKIES_BASE64 or test with a non-music video.';
  ...
  if (/HTTP Error 403|403.*Forbidden/i.test(raw))
    return 'YouTube blocked the download request (HTTP 403). The stream URL expired or bot-detection triggered. Please try again.';
...
export abstract class BaseExtractor implements IExtractor {
  ...
  protected async fetchInfo(url: string): Promise<YtdlpInfo> {
    ...
    // Guard: if yt-dlp returned JSON but with empty formats, it means YouTube
    // returned a response but with no streamable formats — typically bot detection.
    if (Array.isArray(info.formats) && info.formats.length === 0) {
      throw new Error(
        'Could not retrieve video formats from YouTube. ' +
        'This is usually caused by YouTube bot-detection on server IPs. ' +
        'Ensure YT_COOKIES_BASE64 is set with a fresh YouTube login session.'
      );
    }
    return info;
  }
