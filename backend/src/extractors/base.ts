...
export abstract class BaseExtractor implements IExtractor {
  ...
  protected async fetchInfo(url: string): Promise<YtdlpInfo> {
    ...
    // Check playability status before checking formats
    // yt-dlp includes playability_status in the JSON response
    const status = info.playability_status?.toUpperCase() ?? '';
    console.log(`[yt-dlp] playability_status: ${status || 'not set'}, formats: ${info.formats?.length ?? 0}`);

    if (status === 'LOGIN_REQUIRED') {
      throw new Error('YouTube requires sign-in for this video. Your YT_COOKIES_BASE64 may be expired — please re-export from a logged-in Chrome session.');
    }
    if (status === 'UNPLAYABLE') {
      throw new Error('This video is unplayable (DRM or music label restriction). Major Indian music labels block server-side downloading.');
    }
    if (status === 'ERROR') {
      throw new Error('YouTube returned an error for this video. It may be removed or region-restricted.');
    }

    // If formats are empty but status is OK, still try to serve best-quality fallback
    if (Array.isArray(info.formats) && info.formats.length === 0) {
      console.warn('[yt-dlp] Empty formats list despite OK status — using best-effort fallback');
      // Don't throw — let buildFormats return a best-effort option
    }
